...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Evaluate openset predictions.")
    parser.add_argument("--dataset", default="MER2026OV", help="Dataset name used for evaluation.")
    parser.add_argument("--label-csv", default=None, help="Override ground-truth csv path.")
    parser.add_argument(
        "--result-glob",
        default=None,
        help="Glob pattern used to locate prediction npz files. Defaults to output/results-{dataset}/*/*.npz",
    )
    args = parser.parse_args()

    # 1. read ground truth
    label_csv = args.label_csv if args.label_csv not in [None, ""] else config.PATH_TO_LABEL[args.dataset]
    name2gt = {}
    ...
    for _, row in df.iterrows():
        ...
        name2gt[name] = ", ".join(openset)

    # load model
    llm, tokenizer, sampling_params = func_read_batch_calling_model(modelname='Qwen25')

    # 2. calculate results
    result_glob = args.result_glob if args.result_glob not in [None, ""] else f'output/results-{args.dataset.lower()}/*/*.npz'
    for result_path in glob.glob(result_glob):
        ...
        level1_fscore, _, _ = wheel_metric_calculation(name2gt=name2gt, name2pred=name2pred, inter_print=False)
        print (modelname, '& %.2f \\\\' %(level1_fscore*100))
