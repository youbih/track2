...
def get_ckpt3_candidates(ckpt3_root, inference_cfg):
    ...
    if inference_cfg.test_epoch == 'best':
        best_ckpt = os.path.join(ckpt3_root, "checkpoint_best.pth")
        assert os.path.exists(best_ckpt), "Error: checkpoint_best.pth does not exist!"
        return [best_ckpt]
...
def get_face_or_frame(datasets_cfg, outside_face_or_frame):
    ...
    if 'human' in datasets_cfg:
        face_or_frame_candidates.append(datasets_cfg['human'].face_or_frame)
...
def get_name2cls(dataset, datasets_cfg=None, model_cfg=None):
    dataset_cfg = None
    if datasets_cfg is not None:
        if dataset.lower() in datasets_cfg:
            dataset_cfg = datasets_cfg[dataset.lower()]
        elif 'human' in datasets_cfg:
            dataset_cfg = datasets_cfg['human']

    if dataset == 'MER2026OV':        return MER2026OV_Dataset(dataset_cfg=dataset_cfg, model_cfg=model_cfg)
    if dataset == 'Human':            return Human_Dataset(dataset_cfg=dataset_cfg, model_cfg=model_cfg)
...
if __name__ == "__main__":
    ...
    for ii, ckpt_3 in enumerate(whole_ckpt3s):
        ...
        for dataset in process_datasets:
            ...
            dataset_cls = get_name2cls(dataset, datasets_cfg=datasets_cfg, model_cfg=model_cfg)
